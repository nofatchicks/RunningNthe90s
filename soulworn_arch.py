...
# ║  ARCH LINUX EDITION — pacman/yay · btw                                 ║
# ╚══════════════════════════════════════════════════════════════════════════╝
"""
SoulWoRn v5 — Arch Linux CLI Edition
Blood Red + Royal Purple · Triple AI fallback · Script kiddie friendly
Samsung S8+ · Pixels · MTK · Any Android 2012+ · NO telemetry
Arch Linux / Manjaro / EndeavourOS / BlackArch — pacman + yay
"""
...
def adb(*args, timeout=15):
    ...
    try:
        ...
        return 1, "", "adb not found — pacman -S android-tools"
...
def _detect_aur():
    for helper in ["yay", "paru", "trizen", "pikaur"]:
        if _has_cmd(helper): return helper
    return None

def check_deps():
    missing = []
    if not _has_cmd("adb"):
        missing.append("adb      → sudo pacman -S android-tools")
    if not _has_cmd("fastboot"):
        missing.append("fastboot → sudo pacman -S android-tools")
    if not _has_cmd("python3"):
        missing.append("python3  → sudo pacman -S python")
    return missing
...
def ghosteey_claude(prompt, history, api_key):
    ...
    try:
        ...
        raise Exception("anthropic not installed — pip install anthropic --break-system-packages")
...
# ══════════════════════════════════════════════════════════════════════════════
#  ARCH LINUX SETUP HELPER
# ══════════════════════════════════════════════════════════════════════════════

def _pacman(*args, sudo=True):
    prefix = ["sudo"] if sudo else []
    cmd = prefix + ["pacman"] + list(args)
    try:
...
def _yay(*args):
    aur = _detect_aur()
    if not aur: return 1, "", "no AUR helper — install yay: https://aur.archlinux.org/yay.git"
    try:
        r = subprocess.run([aur] + list(args), capture_output=True, text=True)
        return r.returncode, r.stdout.strip(), r.stderr.strip()
    except FileNotFoundError:
        return 1, "", f"{aur} not found"

def arch_install_deps():
    """One-shot install all SoulWoRn deps on Arch."""
    core = [
        "python", "python-pip", "android-tools",
        "git", "curl", "wget", "python-pyqt5",
        "python-requests", "python-pyserial", "base-devel",
    ]
    info("running: sudo pacman -Syu --needed " + " ".join(core))
    subprocess.run(["sudo", "pacman", "-Syu", "--noconfirm", "--needed"] + core)
    subprocess.run(["pip", "install", "--break-system-packages",
                    "anthropic", "pyserial", "requests"])
    success("Arch deps installed")

def arch_setup_menu():
    clear()
    hr_double(C.BLOOD)
    center_text("🏹  A R C H  S E T U P", C.ROYAL)
    hr_double(C.BLOOD)
    print()
    aur = _detect_aur()
    print(f"  {c(C.BLOOD,'pacman:')}      {c(C.GREEN,'✓') if _has_cmd('pacman') else c(C.RED,'✗ not arch??')}")
    print(f"  {c(C.BLOOD,'AUR helper:')} {c(C.GREEN, aur+' ✓') if aur else c(C.GOLD,'not found')}")
    print()
    print(f"  {c(C.BLOOD,'[1]')}  install all SoulWoRn deps")
    print(f"  {c(C.BLOOD,'[2]')}  install yay AUR helper")
    print(f"  {c(C.BLOOD,'[3]')}  add BlackArch repo  {c(C.GREY,'(3000+ hacking tools)')}")
    print(f"  {c(C.BLOOD,'[4]')}  install hacking tools menu")
    print(f"  {c(C.BLOOD,'[5]')}  full dep status check")
    print(f"  {c(C.BLOOD,'[0]')}  back")
    print()
    ch = input(c(C.ROYAL, "  arch ❯ ")).strip()
    if ch == "0": return
        ...
        if _has_cmd("yay"):
            success("yay already installed"); pause(); return
        info("cloning yay from AUR...")
        subprocess.run(["sudo", "pacman", "-S", "--needed", "--noconfirm", "git", "base-devel"])
        subprocess.run("git clone https://aur.archlinux.org/yay.git /tmp/yay_build && "
                       "cd /tmp/yay_build && makepkg -si --noconfirm && "
                       "rm -rf /tmp/yay_build", shell=True)
        success("yay installed!!") if _has_cmd("yay") else error("failed — try manually")
        pause()

    elif ch == "3":
        warn("adding BlackArch repo — ~3000 hacking tools")
        if not confirm("proceed?"): return
        subprocess.run("curl -O https://blackarch.org/strap.sh && "
                       "chmod +x strap.sh && sudo ./strap.sh && "
                       "rm strap.sh && sudo pacman -Syu --noconfirm", shell=True)
        success("BlackArch added — pacman -Ss <tool> to search")
        pause()

    elif ch == "4":
        tools = {
            "1":  (["nmap"],                   "nmap"),
            "2":  (["wireshark-qt"],            "wireshark"),
            "3":  (["aircrack-ng"],             "aircrack-ng"),
            ...
            "8":  (["hydra"],                   "hydra"),
            "9":  (["binwalk"],                 "binwalk"),
            "10": (["python-frida"],            "frida"),
            "11": (["android-apktool"],         "apktool"),
            "a":  (["nmap","aircrack-ng","john","hashcat","sqlmap","hydra","binwalk"], "ALL"),
        }
        print()
        for k, (pkgs, name) in tools.items():
            print(f"  {c(C.BLOOD, f'[{k}]')}  {c(C.LAVENDER, name)}")
        print(f"  {c(C.BLOOD,'[0]')}  cancel")
        tch = input(c(C.ROYAL, "  ❯ ")).strip()
        if tch == "0" or tch not in tools: return
        pkgs, name = tools[tch]
        info(f"installing {name}...")
        code, _, _ = _pacman("-S", "--needed", "--noconfirm", *pkgs)
        if code != 0:
            info("trying AUR helper...")
            _yay("-S", "--needed", "--noconfirm", *pkgs)
        success(f"{name} done")
        pause()
...
def _arch_dep_status():
    clear()
    hr(color=C.BLOOD)
    center_text("DEP STATUS — ARCH", C.ROYAL)
    hr(color=C.BLOOD)
    print()
    checks = [
        ("python3","python3"), ("pip","pip"),
        ("adb","adb (android-tools)"), ("fastboot","fastboot"),
        ("git","git"), ("pacman","pacman"), ("yay","yay (AUR)"),
        ("nmap","nmap"), ("aircrack-ng","aircrack-ng"),
        ("john","john"), ("hashcat","hashcat"),
        ("sqlmap","sqlmap"), ("frida","frida"),
    ]
    for cmd, name in checks:
        found = _has_cmd(cmd)
        print(f"  {c(C.BLOOD, name):<30}"
              f"{c(C.GREEN,'✓  installed') if found else c(C.GREY,'–  not installed')}")
    print()
    print(c(C.ROYAL, "  python packages:"))
    for pkg in ["anthropic", "requests", "serial"]:
        try:
            __import__(pkg)
            print(f"  {c(C.BLOOD, pkg):<30}{c(C.GREEN,'✓  installed')}")
        except ImportError:
            print(f"  {c(C.BLOOD, pkg):<30}{c(C.GREY,'–  not installed')}")
    print()
    print(c(C.ROYAL, "  AI keys:"))
    for label, path in [("Claude", KEY_FILE), ("Groq", GROQ_KEY_FILE)]:
        key = _load_key(path)
        status = c(C.GREEN, f"✓  ({key[:12]}...)") if key else c(C.GREY, "–  not set")
        print(f"  {c(C.BLOOD, label):<30}{status}")
...
def cli_main():
    ...
    while True:
        ...
        menu = [
            ...
            ("k",  "AI key management"),
            ("x",  "arch linux setup",         "pacman · yay · BlackArch · deps"),
            ("q",  "quit"),
        ]
        for item in menu:
            key  = item[0]
